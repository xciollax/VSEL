...
QString Video::toLine() {
    char sep = '\t';
    return name % sep % QString::number(midiNote) % sep % QString::number(loopAtEnd) % sep % QString::number(blurType) % sep % QString::number(zoomType);
}
...
bool Video::fromLine(QString line) {
    ...
    if(al.size() == 5 && (this->name == "" || this->name == al.value(0))) {
        ret = true;
